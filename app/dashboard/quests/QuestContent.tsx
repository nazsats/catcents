...
export default function QuestContent({ initialQuests }: QuestContentProps) {
  ...
  const fetchUserData = async (address: string) => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        setReferralLink(data.referralLink || `${process.env.NEXT_PUBLIC_BASE_URL || window.location.origin}/?ref=${address}`);
      } else {
        const newReferralLink = `${process.env.NEXT_PUBLIC_BASE_URL || window.location.origin}/?ref=${address}`;
  ...
  const handleTaskStart = async (quest: QuestData) => {
  ...
  useEffect(() => {
    ...
    if (!account) {
      console.log('No account found after loading, redirecting to /');
      router.push('/');
      return;
    }
  ...
  }, [account, loading, searchParams, router]);
